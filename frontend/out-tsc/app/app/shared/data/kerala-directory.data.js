...
export const DEMO_PROVIDERS = KERALA_DISTRICTS.flatMap((district, districtIndex) => {
    const baseNumber = 1000 + districtIndex * 10;
    const createdAt = new Date().toISOString();
    return [
        { id: `provider-demo-${districtIndex + 1}-plumber`, userId: `user-demo-${districtIndex + 1}-plumber`, businessName: district.plumber, ownerName: district.plumberOwner, mobile: `+91-90000${baseNumber}1`, email: `plumber${districtIndex + 1}@smartlocal.app`, category: 'plumber', city: district.city, address: `${district.area}, ${district.city}`, availability: districtIndex % 3 === 0 ? 'busy' : 'available', experienceYears: 5 + (districtIndex % 6), verified: districtIndex % 4 !== 1, rating: Number((4.4 + ((districtIndex + 1) % 5) * 0.1).toFixed(1)), reviewCount: 8 + (districtIndex % 5), responseTimeMinutes: 5 + (districtIndex % 7), highResponseRate: districtIndex % 2 === 0, createdAt },
        { id: `provider-demo-${districtIndex + 1}-electrician`, userId: `user-demo-${districtIndex + 1}-electrician`, businessName: district.electrician, ownerName: district.electricianOwner, mobile: `+91-90000${baseNumber}2`, email: `electrician${districtIndex + 1}@smartlocal.app`, category: 'electrician', city: district.city, address: `${district.area}, ${district.city}`, availability: districtIndex % 5 === 0 ? 'busy' : 'available', experienceYears: 4 + (districtIndex % 7), verified: districtIndex % 4 !== 2, rating: Number((4.3 + ((districtIndex + 2) % 6) * 0.1).toFixed(1)), reviewCount: 6 + (districtIndex % 4), responseTimeMinutes: 6 + (districtIndex % 8), highResponseRate: districtIndex % 3 !== 0, createdAt },
        { id: `provider-demo-${districtIndex + 1}-medical-store`, userId: `user-demo-${districtIndex + 1}-medical-store`, businessName: district.medicalStore, ownerName: `${district.city} Pharmacist`, mobile: `+91-90000${baseNumber}3`, email: `medical${districtIndex + 1}@smartlocal.app`, category: 'medical_store', city: district.city, address: `${district.area}, ${district.city}`, availability: 'available', experienceYears: 6 + (districtIndex % 5), verified: true, rating: Number((4.5 + (districtIndex % 4) * 0.1).toFixed(1)), reviewCount: 11 + (districtIndex % 6), responseTimeMinutes: 4 + (districtIndex % 6), highResponseRate: true, createdAt },
        { id: `provider-demo-${districtIndex + 1}-ambulance`, userId: `user-demo-${districtIndex + 1}-ambulance`, businessName: district.ambulance, ownerName: `${district.city} Dispatch`, mobile: `+91-90000${baseNumber}4`, email: `ambulance${districtIndex + 1}@smartlocal.app`, category: 'ambulance', city: district.city, address: `${district.area}, ${district.city}`, availability: 'available', experienceYears: 7 + (districtIndex % 4), verified: true, rating: Number((4.7 + (districtIndex % 3) * 0.1).toFixed(1)), reviewCount: 14 + (districtIndex % 4), responseTimeMinutes: 3 + (districtIndex % 5), highResponseRate: true, createdAt }
    ];
});
export const DEMO_SERVICES = DEMO_PROVIDERS.map((provider) => ({
    _id: provider.id,
...
//# sourceMappingURL=kerala-directory.data.js.map